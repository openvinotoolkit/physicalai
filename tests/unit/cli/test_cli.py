...
class TestMainDispatch:
    ...
    def test_print_completion_exits_zero(self, capsys: pytest.CaptureFixture[str]) -> None:
        with pytest.raises(SystemExit) as exc:
            main(["--print_completion", "shtab-zsh"])
        assert exc.value.code == 0
        output = capsys.readouterr().out
        assert "AUTOMATICALLY GENERATED by `shtab`" in output
        assert os.path.basename(__import__("sys").argv[0]) in output

    def test_print_completion_uses_invoked_program_name(
        self,
        capsys: pytest.CaptureFixture[str],
    ) -> None:
        with (
            patch("sys.argv", ["pai", "--print_completion", "shtab-bash"]),
